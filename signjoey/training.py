...
class TrainManager:
    ...
    def __init__(self, model: SignModel, config: dict) -> None:
        ...
        #model_checkpoint = load_checkpoint(path=model_path, use_cuda=self.use_cuda)
    ...
    def get_inf_iterator(self, data_loader):
        """Inf data iterator."""
        while True:
            for catimages, depthimages, labels in data_loader:
                yield (catimages, depthimages, labels)

    def train_and_validate(self, train_data1: Dataset, train_data2: Dataset, train_data3: Dataset, train_data4: Dataset, train_data5: Dataset, valid_data: Dataset) -> None:
        """
        ...
        iternum = max(len(train_iter1), len(train_iter2), len(train_iter3), len(train_iter4), len(train_iter5))


        t_it = [train_iter1, train_iter2, train_iter3, train_iter4, train_iter5]
        #embed()
        ...
        for epoch_no in range(self.epochs):
            ...
            for i in range(iternum):
                ...
                try:
                    ...
                    aaa1 = iter(train_iter1)
                    batch1 = next(aaa1)

                try:
                    batch2 = next(aaa2)
                except:
                    aaa2 = iter(train_iter2)
                    batch2 = next(aaa2)
    ...
    def zero_param_grad(self, params):
        for p in params:
            if p.grad is not None:
                p.grad.zero_()
            #p.grad = None

    def _train_batch(self, batch: list(), update: bool = True) -> (Tensor, Tensor):
        """
        ...
        meta_train_set = [batch[0], batch[1], batch[2], batch[3]]
        #meta_train_set = [batch[0], batch[1], batch[2], batch[3]]


        meta_test_bat = batch[4]
        ...
        for bat in meta_train_set:
            ...
            if self.do_translation:
                if self.translation_normalization_mode == "batch":
                    txt_normalization_factor = bat.num_seqs
                elif self.translation_normalization_mode == "tokens":
                    txt_normalization_factor = bat.num_txt_tokens
                else:
            ...
            loss_total_train += total_loss

            self.zero_param_grad(self.model.parameters())
            grads_para = torch.autograd.grad(total_loss, self.model.parameters(), create_graph=True, allow_unused=True)
            fast_weights_FeatEmbder = copy.deepcopy(self.model.state_dict())

            adapted_params = OrderedDict()
            for (key, val), grad in zip(self.model.named_parameters(), grads_para):

                if grad is None:
                    adapted_params[key] = val
                else:
                    adapted_params[key] = val - self.optimizer.param_groups[0]["lr"] * grad
        ...
        loss_total_train.backward(retain_graph=True)
        #loss_total_train.backward()

        #embed()

        loss_test = 0
        #loss_test = loss_total_train


        ori_state = copy.deepcopy(self.model.state_dict())

        for n_scr in range(len(adapted_state_dicts)):
            #continue
            a_dict = adapted_state_dicts[n_scr]

            self.model.load_state_dict(a_dict)
            ...
            if self.do_translation:
                if self.translation_normalization_mode == "batch":
                    txt_normalization_factor = meta_test_bat.num_seqs
                elif self.translation_normalization_mode == "tokens":
                    txt_normalization_factor = meta_test_bat.num_txt_tokens
                else:
                    raise NotImplementedError("Only normalize by 'batch' or 'tokens'")
            ...
            if n_scr == len(adapted_state_dicts)-1:
                total_loss.backward()
            else:
        ...
        loss_all = loss_total_train + loss_test
        ...
        if self.do_recognition:
            self.total_gls_tokens += meta_test_bat.num_gls_tokens
        if self.do_translation:
            self.total_txt_tokens += meta_test_bat.num_txt_tokens

        #return normalized_recognition_loss, normalized_translation_loss
        return loss_total_train, loss_test
...
def train(cfg_file: str) -> None:
    ...
    train_data, dev_data, test_data, gls_vocab, txt_vocab, pos_vocab, s1_data, s2_data, s3_data, s4_data, s5_data, s6_data = load_data(
        data_cfg=cfg["data"]
    )
    ...
    pos_vocab_file = "{}/pos.vocab".format(cfg["training"]["model_dir"])
    pos_vocab.to_file(pos_vocab_file)
    ...
    trainer.train_and_validate(train_data1=s1_data, train_data2=s6_data, train_data3=s3_data, train_data4=s2_data, train_data5=s4_data, valid_data=s5_data)

    # Delete to speed things up as we don't need training data anymore
    del train_data, dev_data, test_data, s1_data, s2_data, s3_data, s4_data, s5_data, s6_data
