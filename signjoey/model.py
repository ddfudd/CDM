...
from signjoey.encoders import Encoder, RecurrentEncoder, TransformerEncoder, TransformerEncoder_d
...
class SignModel(nn.Module):
    ...
    def __init__(
        ...
        #self.l22 = nn.Linear(256, 512)
        self.l3 = nn.Linear(512, 9)
        self.l4 = nn.Linear(512, 9)
    ...
    def forward(
        ...
        ######  add disentangled module

        bb = self.l1(torch.mean(encoder_output, 1))
        bb = self.rl(bb)

        #dd = self.l22(bb)
        #bb = self.l2(bb)
        bb = self.sig(self.l2(bb))
        ...
        encoder_out = encoder_output * bb


        ee = self.l3(torch.mean(encoder_out, 1))
        ee_softmax = self.soft(ee)
        ee_log = self.logsoft(ee)

        logloss = torch.sum(ee_softmax * ee_log)

        #logloss = torch.sum(logloss)


        encoder_id = encoder_output * dd
        ff = self.l4(torch.mean(encoder_id, 1))

        id_loss = self.ce(ff, signer)

        identity_loss = logloss + id_loss
        ...
        return decoder_outputs, gloss_probabilities, identity_loss
    ...
    def get_loss_for_batch(
        ...
        decoder_outputs, gloss_probabilities, id_loss = self.forward(
            sgn=batch.sgn,
            signer=batch.signer,
        ...
        if self.do_translation:
            ...
            word_outputs, _, pos_out, c_m, _, _ = decoder_outputs
            # Calculate Translation Loss

            # add contrastive constraints

            s_c = c_m.size()

            s_mask = torch.eye(s_c[1]).expand((s_c[0], s_c[1], s_c[2]))
            s_mask = s_mask.cuda()

            c_loss = - torch.sum(c_m * s_mask)
            ...
            #aa = np.arange(batch.txt.size()[-1])
            #aa = np.expand_dims(aa, 0)
            #aa = np.tile(aa, [batch.txt.size()[0], 1])
            ...
            translation_loss += 0.0 * pos_loss

            translation_loss += 0.0 * id_loss

            translation_loss += 0.0 * c_loss
