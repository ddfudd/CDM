...
from signjoey.transformer_layers import PositionalEncoding, TransformerDecoderLayer, TransformerDecoderLayer_c
    ...
    def forward(
        ...
        **kwargs
        ...
        for layer in self.layers:
            x, c_matrix = layer(x=x, memory=encoder_output, src_mask=src_mask, trg_mask=trg_mask)

        x = self.layer_norm(x)
        output = self.output_layer(x)
        pos_out = self.output_pos(x)

        c_m = self.ce(c_matrix)

        return output, x, pos_out, c_m, None, None
