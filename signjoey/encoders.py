...
class TransformerEncoder_d(Encoder):
    """
    Transformer Encoder
    """
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.l3 = nn.Linear(hidden_size, 4)
    ...
    def forward(
        ...
        cc = self.l3(torch.mean(encoder_output, 1))
        cc_softmax = self.soft(cc)
        cc_log = self.logsoft(cc)

        logloss = cc_softmax * cc_log

        logloss = torch.sum(logloss)


        #return self.layer_norm(x), None
        return encoder_output, logloss
